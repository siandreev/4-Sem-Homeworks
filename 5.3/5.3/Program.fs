module Logic
    ...
    let dictionaryToFile dictionary =     
         let fsOut = new FileStream(@"C:\Users\Admin\Documents\Учеба\Программирование\f#\ДЗ5\3\Data.dat", FileMode.Open)
    ...
    let consoleReader (dictionary : Map<string, string>) (command : string) =
        ...
        | _ when command = "READ Dictionary FROM FILE" ->
            ...
            let dictionaryFromFile = 
               let fsIn = new FileStream(@"C:\Users\Admin\Documents\Учеба\Программирование\f#\ДЗ5\3\Data.dat",FileMode.Open)
               let res : Map<string,string> = readValue fsIn
               fsIn.Close()
               res 
            if File.Exists(@"C:\Users\Admin\Documents\Учеба\Программирование\f#\ДЗ5\3\Data.dat") then (dictionaryFromFile, dictionaryFromFile) 
